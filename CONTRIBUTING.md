...
That is `fmt-check vet lint migrate-check test-race cover sec secrets build`, in
the order CI runs it. A pull request that has not passed it locally will fail on
the runner, and the runner is slower than you are.
