...
-- one in a transaction, which is enough on this engine, where one writer runs
-- at a time. It is not enough on PostgreSQL, where two concurrent enrolments
-- for one subject do not see each other's rows and both end up live, only one
-- of which the user holds. The enrolment tickets have had a partial unique
-- index against exactly that since 0003; this is its counterpart, and it is
-- declared for both engines because the two schemas are kept identical and
-- because a constraint that exists on one engine only is a constraint nobody
-- can rely on.
--
...
UPDATE totp_secrets
SET revoked_at = strftime('%Y-%m-%dT%H:%M:%f000000Z', 'now')
