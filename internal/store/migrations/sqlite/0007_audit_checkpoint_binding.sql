...
-- whoever owns the database file can drop a trigger.
--
...
CREATE TRIGGER audit_checkpoints_marker_guard
BEFORE INSERT ON audit_checkpoints
WHEN NOT EXISTS (
    SELECT 1 FROM audit_log
    WHERE seq = NEW.audit_seq
      AND json_extract(detail, '$.action') = 'retention_prune'
      AND json_extract(detail, '$.pruned_through_seq') = NEW.pruned_through_seq
      AND json_extract(detail, '$.pruned_through_hash') = lower(hex(NEW.pruned_through_hash))
)
BEGIN
    SELECT RAISE(ABORT, 'audit_checkpoints is append-only and takes no row that the marker it names does not attest');
END;

-- The boundary has to be an entry that is still there, holding the hash the
-- checkpoint claims for it. The trim inserts the checkpoint before it deletes
-- anything, so an honest one always satisfies this; a row naming a boundary
-- nobody can check is the shape a forgery takes.
CREATE TRIGGER audit_checkpoints_boundary_guard
BEFORE INSERT ON audit_checkpoints
WHEN NOT EXISTS (
    SELECT 1 FROM audit_log
...
BEGIN
    SELECT RAISE(ABORT, 'audit_checkpoints is append-only and takes no row whose boundary entry is not present');
END;
