...
// The PostgreSQL counterpart of the SQLite ticket tests. Same properties, same
// order, so a divergence between the two engines shows up as a diff between
// these two files rather than as a gap in one of them.
...
func TestIssuingASecondTicketRevokesTheFirst(t *testing.T) {
	...
	if n := countRows(t, s, `SELECT COUNT(*) FROM enrolment_tickets WHERE tenant_id = $1`, "tenant-a"); n != 2 {
